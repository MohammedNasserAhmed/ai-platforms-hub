...
function sanitizePayload(req: express.Request, _res: express.Response, next: express.NextFunction){
...
app.use(cors({
	origin: (origin, callback) => {
		...
		if (allowedOrigins.includes(origin) || /localhost:\d+$/.test(origin)) return callback(null, true);
		return callback(new Error('Not allowed by CORS'));
	}
}));
app.use(morgan('dev'));
// HTTP logging with request IDs
const pinoHttp: any = (pinoHttpModule as any).default || (pinoHttpModule as any);
app.use(pinoHttp({
	logger,
	genReqId: (req: any, _res: any) => (req.headers['x-request-id'] as string) || randomUUID(),
	customProps: (req: any) => ({ requestId: (req as any).id })
}));
...
const upload = multer({ storage, limits: { fileSize: 2 * 1024 * 1024 }, fileFilter: (_req: Request, file: any, cb: any)=> {
	if(!allowedMime.includes(file.mimetype)) return cb(new Error('Unsupported file type'));
	cb(null, true);
} });

app.post('/upload/logo', authMiddleware(process.env.JWT_SECRET!), upload.single('file'), (req: Request & { file?: any }, res)=> {
